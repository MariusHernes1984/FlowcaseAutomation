"""Machine-verifiable checks against a transcript."""
...
from flowcase_web.eval.schema import GoldItem, MachineCheck, MachineChecks, Transcript
...
def run_machine_checks(item: GoldItem, transcript: Transcript) -> MachineChecks:
    ...
    if item.must_mention_any:
        hit = next((n for n in item.must_mention_any if n.lower() in text_lower), None)
        checks.append(
            MachineCheck(
